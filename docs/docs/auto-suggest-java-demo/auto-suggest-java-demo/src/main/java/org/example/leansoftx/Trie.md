# 基础信息

|      |      |
|------|------|
| 名称 | Trie |
| 编码语言 | .java |
| 代码路径 | auto-suggest-java-demo/src/main/java/org/example/leansoftx/Trie.java |
| 包名 | org.example.leansoftx |
| 依赖项 | ['java.util'] |
| 概述说明 | Trie树实现，支持插入、自动补全、拼写建议和结构打印功能。 |

# 说明

该内容描述了一个Trie字典树结构的实现。主要功能包括：初始化Trie树根节点；插入单词并检查是否已存在；根据前缀自动建议完整单词；获取所有单词或特定前缀的单词列表；打印Trie树结构；提供拼写建议并计算编辑距离。通过TrieNode节点存储字符和子节点，支持字符级遍历和树形结构可视化。拼写建议基于Levenshtein编辑距离算法，返回与输入词距离不超过2的候选词。

# 类列表 Class Summary

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| Trie | class | Trie树实现，支持插入、自动补全、拼写建议和结构打印功能。 |



## 类 Trie

|      |      |
|------|------|
| 访问范围 | public |
| 类型 | class |
| 名称 | Trie |
| 说明 | Trie树实现，支持插入、自动补全、拼写建议和结构打印功能。 |


### UML类图

```mermaid
classDiagram
    class Trie {
        -TrieNode root
        +Trie()
        +boolean insert(String word)
        +List~String~ autoSuggest(String prefix)
        +List~String~ getAllWordsWithPrefix(TrieNode node, String prefix)
        +List~String~ getAllWords()
        +void printTrieStructure()
        -void _printTrieNodes(TrieNode root, String format, boolean isLastChild)
        +List~String~ getSpellingSuggestions(String word)
        +static int levenshteinDistance(String s, String t)
    }

    class TrieNode {
        -char value
        -Map~Character, TrieNode~ children
        -boolean isEndOfWord
        +TrieNode(char value)
        +boolean hasChild(char c)
    }

    Trie --> TrieNode : contains
```

该代码实现了一个字典树(Trie)数据结构，用于高效存储和检索字符串。Trie类包含插入单词、自动补全建议、获取所有单词、打印树结构以及拼写建议等功能。TrieNode类表示树的节点，包含字符值、子节点映射和单词结束标志。核心功能包括通过levenshteinDistance方法计算编辑距离，实现拼写纠错建议。类图展示了Trie与TrieNode之间的包含关系，以及各自的方法和属性。


### 内部方法调用关系图

```mermaid
graph TD
    A["类Trie"]
    B["属性: TrieNode root"]
    C["构造方法: Trie()"]
    D["方法: boolean insert(String word)"]
    E["方法: List<String> autoSuggest(String prefix)"]
    F["方法: List<String> getAllWordsWithPrefix(TrieNode node, String prefix)"]
    G["方法: List<String> getAllWords()"]
    H["方法: void printTrieStructure()"]
    I["私有方法: _printTrieNodes(TrieNode root, String format, boolean isLastChild)"]
    J["方法: List<String> getSpellingSuggestions(String word)"]
    K["静态方法: levenshteinDistance(String s, String t)"]

    A --> B
    A --> C
    A --> D
    A --> E
    A --> F
    A --> G
    A --> H
    H --> I
    A --> J
    J --> K
    E --> F
    G --> F
    J --> F
    D -->|调用| B
    E -->|调用| B
    I -->|递归调用| I
```

这段代码实现了一个Trie（前缀树）数据结构，主要用于高效存储和检索字符串。核心功能包括插入单词、自动补全建议、获取所有单词、打印树结构以及拼写建议。流程图展示了类的主要方法和属性关系，其中insert()和autoSuggest()是核心操作，printTrieStructure()通过递归打印树形结构，getSpellingSuggestions()利用编辑距离算法提供拼写纠正建议。静态方法levenshteinDistance()计算两个字符串的编辑距离，用于拼写建议功能。

### 字段列表 Field List

| 名称  | 类型  | 说明 |
|-------|-------|------|
| root | TrieNode | 私有根节点变量root |

### 方法列表 Method List

| 名称  | 类型  | 说明 |
|-------|-------|------|
| getAllWordsWithPrefix | List<String> | 方法定义：获取指定前缀的所有单词，返回列表，当前返回空。 |
| levenshteinDistance | int | 计算两个字符串的编辑距离。 |
| printTrieStructure | void | 打印Trie树结构，显示根节点及子节点。 |
| getAllWords | List<String> | 获取所有单词列表 |
| insert | boolean | 字典树插入单词方法：遍历字符，无子节点则创建，最后标记单词结束。 |
| _printTrieNodes | void | 递归打印Trie树节点，按字母排序显示子节点，用符号表示层级关系。 |
| autoSuggest | List<String> | 方法根据前缀在字典树中搜索匹配的单词列表。 |
| getSpellingSuggestions | List<String> | 获取拼写建议：基于前缀和编辑距离筛选相似词。 |




