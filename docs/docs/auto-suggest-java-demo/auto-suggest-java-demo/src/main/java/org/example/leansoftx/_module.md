...
该流程图展示了 `leansoftx` 目录下的文件结构，包含了三个主要文件：`Trie.java`、`TrieNode.java` 和 `Main.java`。这些文件都直接位于 `leansoftx` 目录下，没有进一步的子目录层级。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [Main.java](Main.md) | file | Java利用Trie实现字典功能，支持搜索、补全、删除和拼写建议。 |
| [TrieNode.java](TrieNode.md) | file | TrieNode类含子节点映射、结束标志和字符值，支持查询子节点。 |
| [Trie.java](Trie.md) | file | Trie树支持插入、自动补全、拼写建议和打印树结构。 |
