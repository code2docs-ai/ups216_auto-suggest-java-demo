...
该流程图展示了 `auto-suggest-java-demo` 项目中 `src/main/java/org/example/leansoftx` 目录下的文件层级关系。`org` 目录包含 `example` 子目录，`example` 目录下又包含 `leansoftx` 子目录，`leansoftx` 目录中包含 `Trie.java`、`TrieNode.java` 和 `Main.java` 三个文件。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [example](example/_module.md) | package | Trie类实现高效字符串处理，支持插入、搜索、删除、自动补全和拼写建议功能。 |
