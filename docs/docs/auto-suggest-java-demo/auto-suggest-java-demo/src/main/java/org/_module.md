...
该流程图展示了 `auto-suggest-java-demo` 项目中的 Java 文件层级关系。从 `org` 目录开始，依次进入 `example` 和 `leansoftx` 子目录，最终显示了 `Trie.java`、`TrieNode.java` 和 `Main.java` 三个文件。这些文件构成了项目的主要代码结构，反映了代码的组织方式。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [example](example/_module.md) | package | Trie类实现插入、自动建议、拼写建议和打印功能，TrieNode类表示节点，支持字符串匹配和前缀搜索。 |
